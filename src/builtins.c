...
extern int last_return_code; // Assuming you have a global variable for the last return code

int echo(char *message)
{
    char *start = message;
    char prev_char = 0; // To keep track of the previous character
        ...
        else if (*start == '$' && *(start + 1) == '{')
{
    ...
    prev_char = *(start - 1);
        ...
        {
            // If the current character and previous character are spaces, skip this iteration
            if (!(*start == ' ' && prev_char == ' '))
            {
                putchar(*start);
            }
